...
(function ($) {
    ...
    Danmu.prototype.addDanmu = function (arg) {
        console.log("进入addDanmu函数,arg:"+JSON.stringify(arg)+"--类型："+typeof (arg)+"---大小:"+arg.size);
        if (arg instanceof Array) {
            console.log("arg是Array类型");
            for (var i in arg) {
                console.log("进入循环条件弹幕到弹幕池");
                if (this.$element.data("danmuList")[arg[i]["time"]]) {
                    console.log("arg的time属性不为空");

                    this.$element.data("danmuList")[arg[i]["time"]].push(arg[i]);
                }
                else {
                    console.log("arg的time属性为空");
