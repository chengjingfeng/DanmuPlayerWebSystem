...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.getDanmu = function () {
            ...
            xhr.open('GET', that.options.urlToGetDanmu)
            ...
            xhr.send({})
             **/
            console.log("进入getDanmu函数")
            $.get(that.options.urlToGetDanmu, function (data, status) {
                console.log("异步调用获得的data类型:"+typeof(data)+"------data:"+JSON.stringify(data));
                var str=JSON.stringify(data);
                var danmuFromMongoDB = eval(str);
                console.log("danmuFromMongoDB类型:"+typeof(danmuFromMongoDB)+"--------danmuFromMongoDB:"+danmuFromMongoDB+"----长度:"+danmuFromMongoDB.length);

                for (var i = 0; i < danmuFromMongoDB.length; i++) {
                    console.log("进入循环：第"+i+"个弹幕danmuFromMongoDB[i].stringify:"+JSON.stringify(danmuFromMongoDB[i]));
                    try {
                        var danmuLs = eval('(' +JSON.stringify( danmuFromMongoDB[i]) + ')');
                    } catch (e) {
                        continue;
                    }
                    console.log("danmuLs："+danmuLs);
